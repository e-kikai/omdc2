prawn_document do |pdf|
  @companies.each do |c|
    ...
    pdf.text "#{c.name} 御中", {size: 16}
    pdf.stroke_line [12.mm, (132.5- 23).mm], [70.mm, (132.5- 23).mm]

    pdf.text "#{c.no}", size: 12

    pdf.text "#{@open_now.name}", size: 14

    pdf.table [["御請求額は上記の通りです。", "平成29年06月07日までにご入金下さい。"]], {
      header: true,
    } do |t|
    end


    pdf.bounding_box([8.mm, 100.mm], width: 178.mm, height: 60.mm) do
      # pdf.stroke_bounds

      pdf.default_leading 8

      pdf.text "御請求額は上記の通りです。", size: 12
      pdf.text "平成29年06月07日までにご入金下さい。", size: 14
      ...
      pdf.text "略称 キカイダンチテンジジョウ", size: 12

      pdf.font "vendor/assets/fonts/VL-PGothic-Regular.ttf"
      pdf.text "お願い:お振り込みいただく場合は、振込手数料は貴社でご負担下さい。", size: 12

      pdf.text "註:落札請求額と入札支払額の双方がある場合、相殺し差額決済とします。", size: 12
    end
    ...
    #     t.columns(14).style(width: 50, align: :left)
    #
    #     t.row(0).style(align: :left)
    #
