prawn_document do |pdf|
  @companies.each do |c|
    ...
    pdf.bounding_box([0, 92.mm], width: 178.mm, height: 80.mm) do
      ...
      pdf.text "平成#{@open_now.payment_date.strftime("%Y").to_i - 1988}年#{@open_now.payment_date.strftime("%m月%d日")}までにご入金下さい。", size: 16
      pdf.text ""
      pdf.font "vendor/assets/fonts/VL-PGothic-Regular.ttf"
      ...
      pdf.text ""
      pdf.text "註:落札請求額と入札支払額の双方がある場合、相殺し差額決済とします。", size: 12
