["No.", "大ジャンルID", "大ジャンル", "中ジャンルID", "中ジャンル", "ジャンルID", "ジャンル",
   "商品名", "メーカー", "型式", "最低入札金額", "出品会社No.", "出品会社", "出品エリア", "出庫日時",
    "入札数",  "落札金額", "落札会社No.", "落札会社", "同額札"].to_csv +
@products.sum do |p|
  res = [p.list_no, p.xl_genre.id, p.xl_genre.name, p.large_genre.id, p.large_genre.name, p.genre_id, p.genre.name,
    p.name, p.maker, p.model, p.min_price, p.company.no, p.company.name, p.area_name, p.carryout_at]
  res += if p.bid?
